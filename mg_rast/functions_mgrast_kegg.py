...
def match_wgt_metadata_with_ontology_terms(input_file):
    ...
    with open(input_file, 'r') as file_with_metadata_retrieved:
        ...
        envo_ncbi_ids = {}
        ko_terms_ncbi_ids = {}
        envo_ko_terms_ncbi_ids_per_sample = {}
        dictionary_of_envo_terms = {}
        ...
        for line in file_with_metadata_retrieved:
            ...
            # Now the KOs retrieved are kept in a list; the first element is empty, i.e. ''   
            # Keep the taxa and the KOs found as lists
            taxa_found = attributes[6].split("|")
            sample_kegg_terms = attributes[7].split("|")  #[1:]
                       
            # This is a dictionary for the kegg terms found in the sample under study 
            sample_ko = {}
            for ko in sample_kegg_terms:
                if ko != '':
                    ko = ko.split(":")
                    ko_term = ko[0]
                    ko_abundance = float(ko[-1])
                    if ko_term in sample_ko.keys():
                        sample_ko[ko_term] += ko_abundance
                    else:
                        sample_ko[ko_term] = ko_abundance
            ...
            envo_ko_terms_ncbi_ids_per_sample[sample_id] = {}
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['project'] = sample_project
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['biome'] = sample_biome
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['feature'] = sample_feature
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['material'] = sample_material            
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['KOs'] = sample_ko
            envo_ko_terms_ncbi_ids_per_sample[sample_id]['taxa'] = sample_ncbi_id_taxa
            
            # print(envo_ko_terms_ncbi_ids_per_sample[sample_id])
            metagenome_counter += 1
            
    file_with_metadata_retrieved.close()
    
    return envo_ko_terms_ncbi_ids_per_sample
    # returning:
    # envo_ko_terms_ncbi_ids_per_sample

def get_backgrounds_wgs_case(envo_ko_terms_ncbi_ids_per_sample):
    
    # Build three dictionaries for ENVO, NCBI_IDs and KOs correspondingly
    
    envo_backgrounds = {}
    ncbi_ids_backgrounds = {}
    kegg_terms_backgrounds = {}
    
    # Parse the dictionary returned from the match_wgt_metadata_with_ontology_terms() function
    for sample, metadata in envo_ko_terms_ncbi_ids_per_sample.items():
        ...
        for entry, value in metadata.items():
            ...
            # And the processes    
                ...
                for ko, abundance in metadata['KOs'].items(): 
                    if abundance > 0: 
                        if ko not in kegg_terms_backgrounds.keys(): 
                            kegg_terms_backgrounds[ko] = 1 
                        else: 
                            kegg_terms_backgrounds[ko] += 1 

    return envo_backgrounds, ncbi_ids_backgrounds, kegg_terms_backgrounds
    # returning:
    # envo_backgrounds, ncbi_ids_backgrounds, kegg_terms_backgrounds

def count_cooccurences(envo_ko_terms_ncbi_ids_per_sample):
    
    envo_ncbi_ids = {} ; envo_kegg = {} ; kegg_ncbi_ids = {}
    ...
    for sample, metadata in envo_ko_terms_ncbi_ids_per_sample.items():
        ...
        # Parse the KOs found in the sample
        for ko, abundance in metadata['KOs'].items(): 
            ...
            if abundance > 2:
                ...
                for envo in envo_terms:
                    ...
                    if ko not in envo_kegg[envo].keys():
                        envo_kegg[envo][ko] = 1
                    else:
                        envo_kegg[envo][ko] += 1

        # Parse both taxa and KOs of the sample to get their co-occurence
        for ko, ko_abundance in metadata['KOs'].items():
            
            if ko_abundance > 2:
                if ko not in kegg_ncbi_ids.keys():
                    kegg_ncbi_ids[ko] = {}
            ...
            for taxon, taxon_abundance in metadata['taxa'].items():
                if taxon_abundance > 5:
                    if taxon not in kegg_ncbi_ids[ko].keys():
                        kegg_ncbi_ids[ko][taxon] = 1
                    else:
                        kegg_ncbi_ids[ko][taxon] += 1
            
    return envo_ncbi_ids, envo_kegg, kegg_ncbi_ids
    # returning:
    # envo_ncbi_ids, envo_kegg, kegg_ncbi_ids -- the latter is like:
